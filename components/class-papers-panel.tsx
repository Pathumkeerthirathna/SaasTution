...
export function ClassPapersPanel() {
  ...
  const deepLinkPaperId = searchParams.get("paperId");
  ...
  // auto-open the right paper's submissions once the list has loaded.
  useEffect(() => {
    if (hasAutoExpanded.current || !deepLinkPaperId || isLoading) return;
    if (!papers.some((p) => p.id === deepLinkPaperId)) return;

    hasAutoExpanded.current = true;
    setExpandedId(deepLinkPaperId);
    setDetail(null);
    void loadDetail(deepLinkPaperId);
  }, [deepLinkPaperId, isLoading, papers, loadDetail]);

  useFocusHighlight(!detailLoading && detail?.id === deepLinkPaperId);
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5">
                          <h3 className="break-words text-sm font-semibold text-slate-900">{paper.name}</h3>
                          <span
...
function SubmissionsTable({
  ...
  return (
    <>
      {/*
        Below sm there isn't room for this 3-column table without
        horizontal scrolling, so each submission renders as its own card
        instead — same student info, submission status, and mark-entry
        controls, just stacked.
      */}
...
function SubmissionRow({
  ...
  const studentBlock = (
    <>
      <p className="break-words font-semibold text-slate-800">{sub.studentName}</p>
  ...
  const submissionBlock = sub.submitted ? (
  ...
  if (layout === "card") {
    return (
      <div
        id={focusElementId(sub.id)}
        className="scroll-mt-24 rounded-lg border border-slate-200 bg-white p-2.5 text-xs"
      >
        {studentBlock}
  ...
  return (
    <tr id={focusElementId(sub.id)} className="scroll-mt-24 bg-white">
      <td className="px-2 py-2">{studentBlock}</td>
      <td className="px-2 py-2">{submissionBlock}</td>
      <td className="px-2 py-2">{marksBlock}</td>
    </tr>
  );
}
